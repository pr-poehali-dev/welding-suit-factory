"""
CRUD товаров, галерея фото, размерный ряд, авторизация.
Маршрутизация через httpMethod + action в body/query.

GET  /                          — список товаров с фото и размерами
GET  /?action=sizes&product_id= — размерный ряд товара
POST / {action: auth}           — авторизация менеджера
POST / {action: create}         — создать товар
POST / {action: upload}         — загрузить фото → CDN URL
POST / {action: add_image}      — добавить фото в галерею
POST / {action: add_size}       — добавить/обновить размер
PUT  / {id, ...}                — обновить товар
DELETE / ?action=product&id=    — удалить товар
DELETE / ?action=image&id=      — удалить фото
DELETE / ?action=size&id=       — удалить размер
"""
import json
import os
import base64
...
def handler(event: dict, context) -> dict:
    ...
    # ── GET — список товаров или размерный ряд ──
    if method == "GET":
        conn = get_conn()
        cur  = conn.cursor()
